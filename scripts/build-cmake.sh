...
#          cmake-<v>-SHA-256.txt and built with CMake's own ./bootstrap, so no cmake is needed to
#          build cmake. Universal comes from running this twice (x86_64/10.9, arm64/11.0) and
#          scripts/lipo-merge-tree.sh. The compiler is the environment's CC/CXX, which bootstrap
...
# platform: the arch and floor go ONLY to the real build (the options after `--`), never into CFLAGS:
#           bootstrap first compiles a stage-0 cmake that must RUN on this host, and a runner without
#           Rosetta cannot run an x86_64 stage-0.
...
# platform: without --no-system-libs, CMake's own configure finds and links whatever
#           curl/zlib/etc. the build host has installed (pkgsrc, Homebrew, MacPorts) -- which it
#           must never do, since a shipyard-cmake binary is built on ONE box and run on others that
#           lack that package manager or that library at that path. CMAKE_IGNORE_PREFIX_PATH is
#           defence in depth behind it. BUILD_CursesDialog=OFF because ccmake is not shipped and
#           needs curses.
