...
# spec: two injection points, deliberately. $MAVERICKS_RELEASES is the finished records: it
#       exercises the DECISION. $MAVERICKS_RELEASES_RAW is the API's own shape --
#       "<tag><TAB><draft><TAB><body>", the body escaped the way jq's @tsv escapes it -- so the
#       code that PARSES the outside world is code these cases run too. It once was not, and the
#       two bugs that hid behind the higher injection point were both reproduced: an invalid
#       --json field made every night answer PUBLISH in all 14 repos, and any gh failure answered
#       PUBLISH too.
...
# spec: everything below injects the RAW API shape, so the transform that PARSES the outside
#       world actually runs.
P11="$(printf '%s\n' \
...
# spec: the one clause the offline cases cannot reach is the fetch's own request, so pin it
#       directly. `gh release list --json tagName,body` FAILS -- `body` is a `gh release view`
#       field -- and a failed list read as "no releases", which answers PUBLISH every night, in
#       every repo. @tsv is what makes a record exactly one line, which cases above rely on being
#       true of the real API. Full-line comments are stripped first: the script explains the trap
#       it avoids, and the prose naming it must not read as the trap itself.
code="$(sed 's/^[[:space:]]*#.*//' "$S")"
case "$code" in
...
# spec: a gh FAILURE IS A FAILURE, never a decision. Auth expiry, a rate limit, a 5xx, a network
#       blip and a renamed repo all produce empty output, and empty output means PUBLISH. The
#       seam is $MAVERICKS_GH, an explicit path, not a stub on PATH: a stub on PATH is invisible
#       at the call site and would shadow gh for anything else the script ran.
...
# spec: THROUGH REAL jq, WITH THE SCRIPT'S OWN FILTER, so the raw shape the cases above inject is
#       the shape the API actually produces. Skipped where jq is absent (10.9 has none); CI has it.
...
JSON
  ...
  # spec: a RENAMED tag field is how the empty-tag case above breaks in practice: jq yields null,
  #       @tsv renders it empty, and every release disappears.
...
JSON
  ...
  # spec: prose containing a literal backslash-n before the key must not FORGE a marker. @tsv
  #       escapes `\` as well as newlines, so the tempting one-line unescape (gsub of \n alone)
  #       would turn this into a real line break and read a digest out of a sentence.
